// Native example: LottieAssetResolver.cpp
...
(async () => {
  ...
  const [logo, font, resolver1, resolver2] = await Promise.all([
    ...
    fetch('playground/assets/lottie/extensions/resolver1.json').then(r => r.text()),
    fetch('playground/assets/lottie/extensions/resolver2.json').then(r => r.text()),
  ]);
