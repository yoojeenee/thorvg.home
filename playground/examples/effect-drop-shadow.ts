// Native example: EffectDropShadow.cpp
...
(async () => {
  {
    ...
    const response = await fetch('playground/assets/images/thorvg-logo-clear.svg');
  ...
  // Prepare a scene for post effects
  {
    ...
    const response = await fetch('playground/assets/images/152932619-bd3d6921-72df-4f09-856b-f9743ae32a14.svg');
    const svgString = await response.text();
    const svgData = new TextEncoder().encode(svgString);
  ...
  // Prepare a scene for post effects
  {
    ...
    const response = await fetch('playground/assets/images/circles1.svg');
    const svgString = await response.text();
    const svgData = new TextEncoder().encode(svgString);
